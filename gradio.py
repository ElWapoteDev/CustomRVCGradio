...
def get_vc(sid):
    ...
    modelos, indexes = change_choices()
    esta_seleccion = ''

    if len(indexes['choices']) > 0:
        print('Si hay indexes')

        for este_index in indexes['choices']:
            if sid.replace('.pth', '') in este_index:
                esta_seleccion = este_index
                break
            
    print(f'Este index: {esta_seleccion}')

    
    return {"visible": False, "maximum": n_spk, "__type__": "update"}, {"visible": True, "choices": indexes['choices'], "value": esta_seleccion, "__type__": "update"}
...
def seleccionar_tipo(seleccion):
    ...
    return {"visible": YouTube_visible, "__type__": "update"}, {"visible": Local_visible, "__type__": "update"}

def descargar_youtube(enlace, path='/content/descargas'):

    if (not os.path.exists(path=path)):
        os.makedirs(path, exist_ok=True)

    for archivo in os.listdir(path):
        ruta_entera = os.path.join(path, archivo)
        if os.path.isfile(ruta_entera):
            os.remove(ruta_entera)

    try:
        Instancia = YouTube(enlace)

        if os.path.exists(os.path.join(path, f'{Instancia.title.replace(" ", "_")}.mp3')):
            os.remove(os.path.join(path, f'{Instancia.title.replace(" ", "_")}.mp3'))

        if os.path.exists(os.path.join(path, f'{Instancia.title.replace(" ", "_")}.mp4')):
            os.remove(os.path.join(path, f'{Instancia.title.replace(" ", "_")}.mp4'))
        ...
        print(f'Ocurrio el siguiente error con este video: {err}')
        return False

def Separar(AudioPath, TitArchivo, OutputPath='/content/separados'):
    if (not os.path.exists(path=OutputPath)):
    ...
    if os.path.exists(Instrumental) and os.path.exists(Vocal):
        print('Los archivos ya existen')
        return Instrumental, Vocal

    Comando = f'python3 -m demucs --out {OutputPath} --two-stems vocals tracks {AudioPath}'
    subprocess.run(Comando, shell=True)

    Inicio = time.time()

    while not os.path.exists(Vocal):
        #print('Esperando...')
        time.sleep(1) 
...
def Juntar(Vocal, Instrumental, tit_archivo):
    Vocal = AudioSegment.from_file(Vocal)
    Instrumental = AudioSegment.from_file(Instrumental)
    Instrumental = Instrumental[:len(Vocal)]
    Instrumental = Instrumental.overlay(Vocal)

    output_file = f'/content/juntadas/{tit_archivo}_unido.mp3'

    os.makedirs('/content/juntadas', exist_ok=True)

    if os.path.exists(output_file):
        os.remove(output_file)

    Instrumental.export(output_file, format='mp3')

    return output_file

def save_audio(file_path, audio, sample_rate):
    try:
        sf.write(file_path, audio, sample_rate, 'PCM_16')
        print("El archivo de audio se guardó correctamente.")
    except Exception as e:
        print("Error al guardar el archivo de audio:", str(e))

def comenzar_inferencia(
        spk_item,
        input_audio0,
        vc_transform0,
        f0_file,
        ...
        #Aqui estan cosas extras
        enlace_yutuf,
        separaraudio,
        donde_audio
):
    #descarga = ''
    if enlace_youtube != '' and donde_audio == 'YouTube':
        descarga = descargar_youtube(enlace=enlace_yutuf)
        if descarga is not False:
    ...
    if separaraudio == 'Separar':
        ...
        if instrumental != False and vocal != False:
            input_audio0 = vocal
        else:
            print('Ocurrio un error al separar el audio')
            return 'No se pudo separar', (None, None)

    print(enlace_yutuf)
    print('Comenzando la inferencia')

    if torch.cuda.is_available():
        torch.cuda.reset_max_memory_allocated()
        torch.cuda.empty_cache()

    retorno, datos = vc_single(
        sid=spk_item, 
        input_audio_path=input_audio0, 
    ...
    nombre_god = os.path.basename(input_audio0)
    save_path = f'/content/vocales/{nombre_god.replace(".mp3", ".wav")}'
    if os.path.exists(save_path):
        os.remove(save_path)

    os.makedirs('/content/vocales', exist_ok=True)

    save_audio(save_path, datos[1], datos[0])
    
    unido = ''
    if separaraudio == 'Separar':
        try:
            unido = Juntar(save_path, instrumental, os.path.basename(input_audio0))
        except Exception as err:
            print(f'Ocurrio un error al juntar las vocales: {err}')
    else: 
        unido = save_path
    
    return {'value': retorno, "__type__": "update"}, {'value': save_path, "__type__": "update"}, {'value': unido, "__type__": "update"}

def set_fake_ilusion(audio):
    return {'value': audio.name, 'visible': True, "__type__": "update"}

def obtener_id_enlace_drive(enlace_drive):
    inicio_id = enlace_drive.find("/d/") + 3
    fin_id = enlace_drive.find("/view")
    
    if inicio_id > 2 and fin_id > 0:
        id_drive = enlace_drive[inicio_id:fin_id]
    else:
        inicio_id = enlace_drive.find("/d/") + 3
        id_drive = enlace_drive[inicio_id:]
...
def generar_nombre(path):
    archivos = len(os.listdir(path)) + 1
    return f'descarga_({archivos}).zip'

def descomprimir_archivo_zip(archivo_zip, carpeta_destino):
    if not os.path.exists(carpeta_destino):
        os.makedirs(carpeta_destino)
    
    nombre_carpeta = os.path.splitext(os.path.basename(archivo_zip))[0]
    
    carpeta_destino_completa = os.path.join(carpeta_destino, nombre_carpeta)
    
    with zipfile.ZipFile(archivo_zip, 'r') as zip_ref:
        zip_ref.extractall(carpeta_destino_completa)
    
    return carpeta_destino_completa

def download_drive(enlace):
    id_archivo = obtener_id_enlace_drive(enlace)
    if id_archivo is False:
        print('no hay id')
        return False, 'El formato es invalido'

    enlace_descarga = f'https://drive.google.com/uc?id={id_archivo}'    
    salida = '/content/descargas_modelos'

    os.makedirs(salida, exist_ok=True)

    output_god = f'{salida}/{generar_nombre(salida)}'

    try:
        gdown.download(url=enlace_descarga, output=output_god, quiet=False)
    except Exception as err:
        return False, str(err)
    
    if os.path.exists(output_god):
        return True, output_god
    
    return False, 'Ocurrio un error al descargar el modelo'
    ...
    for ruta_actual, carpetas, archivos in os.walk(carpeta):
        for archivo in archivos:
            ...
            if archivo.endswith('.pth'):
                archivos_pth.append(ruta_completa)
            elif archivo.endswith('.index'):
                archivos_index.append(ruta_completa)
    
    return archivos_pth, archivos_index


def mover_archivos(path):
    root_weihts = weight_root
    root_index = index_root

    archivos_pth, archivos_index = obtener_archivos(path)

    pth_elegido = ''
    index_elegido = ''

    if len(archivos_pth) == 0:
        return False, 'No se encontro un pth'
    
    if len(archivos_pth) == 1:
        pth_elegido = archivos_pth[0]
    elif len(archivos_pth) >= 2:
        for pth in archivos_pth:
            if not 'G_' in pth and not 'D_' in pth:
                pth_elegido = pth
                break
    
    if len(archivos_index) == 1:
        index_elegido = archivos_index[0]
    elif len(archivos_index) >= 2:
        for index in archivos_index:
            if not 'trained_' in index:
                index_elegido = index
                break
    ...
    nombre_modelo = os.path.basename(pth_elegido)
    este_log = f'{root_index}/{nombre_modelo.replace(".pth", "")}'
    os.makedirs(este_log, exist_ok=True)

    try:

        if os.path.exists(f'{este_log}/{index_elegido}'):
            shutil.rmtree(este_log)

        if os.path.exists(f'{root_weihts}/{nombre_modelo}'):
            os.remove(f'{root_weihts}/{nombre_modelo}')

        shutil.move(pth_elegido, root_weihts)
        shutil.move(index_elegido, este_log)

        return True, pth_elegido
    except Exception as err:
...
def descargar_modelo_extraer(enlace):
    ...
    if succ == True:
        descomprimido = descomprimir_archivo_zip(info, '/content/modelos_descomprimidos')
...
with gr.Blocks() as app:
    ...
    with gr.Tabs():
        with gr.TabItem('Inferencia Facil'):
            with gr.TabItem('Inferencia'):
                with gr.Row():
                    with gr.Column():
                        gr.Markdown(value='Seleccion de voz')

                        refresh_button = gr.Button(i18n("刷新音色列表和索引路径"), variant="primary")
                        sid00 = gr.Dropdown(label='Voz de inferencia:', choices=sorted(names))
                        file_index22 = gr.Dropdown(
                            label=".index de la voz (se coloca automaticamente si existe):",
                            choices=sorted(index_paths),
                            interactive=True,
                        )
                        refresh_button.click(
                            fn=change_choices, inputs=[], outputs=[sid00, file_index22]
                        )

                        gr.Markdown(value='Seleccion de audio')

                        SepararAudio = gr.Radio(
                            label='Separar instrumental y vocal automaticamente (Por si tiene instrumental el audio):',
                            choices=['Separar', 'Mantener'], # Fork Feature. Add Crepe-Tiny
                            value="Separar",
                            interactive=True,
                        )

                        tipo_archivo = gr.Dropdown(
                            label='De donde usaras el audio:',
                        ...
                        enlace_youtube = gr.Textbox(
                            value='https://www.youtube.com/watch?v=DjboKAyrvPg&ab_channel=RanchoHumilde',
                            lines=1,
                            max_lines=1,
                            placeholder='Enlace de youtube',
                            label='Enlace de youtube de la cancion:',
                            info='Enlace de youtube de la cancion, a la que quieres cambiarle la voz',
                            show_label=True,
                            visible=True,
                            interactive=True
                        )

                        archivo_personalziado = gr.File(
                            label='Si no usas audio de youtube, puedes subir un archivo de tu sistema:',
                            file_types=['mp3', 'wav'],
                            interactive=True,
                            visible=False,
                            type='file'
                        ...
                        fake_vision = gr.Textbox(
                            label='Ignora esto (test)',
                            visible=False
                        )

                        archivo_personalziado.change(
                            fn=set_fake_ilusion,
                            inputs=[archivo_personalziado],
                            outputs=[fake_vision]
                        )



                        tipo_archivo.change(fn=seleccionar_tipo, inputs=[tipo_archivo], outputs=[enlace_youtube, archivo_personalziado])
                    ...
                    sid00.change(
                        fn=get_vc,
                        inputs=[sid00],
                        outputs=[spk_item1, file_index22],
                    )
                    
                    with gr.Column():
                        gr.Markdown(value='Configuraciones')
                        
                        vc_transform0 = gr.Slider(
                                label="Cambiar de tono la voz (número entero, cantidad de saltos de tono, subir una octava: 12, bajar una octava: -12):",
                                value=0,
                                minimum=-36,
                                maximum=36,
                                step=6,
                        ...
                        f0method00 = gr.Radio(
                            label=i18n(
                                "Selecciona el algoritmo de extracción de tono ('pm': extracción más rápida pero calidad de voz inferior; 'harvest': mejor calidad de bajos pero extremadamente lento; 'crepe': mejor calidad pero intensivo en GPU)."
                            ),
                        ...
                        crepe_hop_length1 = gr.Slider(
                            ...
                            maximum=512,
                            step=1,
                            label='Crepe Hop Length (Solo aplica a crepe): La longitud de salto hace referencia al tiempo que tarda el hablante en cambiar a un tono drástico. Las longitudes de salto más cortas tardan más tiempo en inferir, pero son más precisas en cuanto al tono.',
                            value=160,
                            interactive=True
                        )
                        ...
                        f0_filee = gr.File(label=i18n("F0曲线文件, 可选, 一行一个音高, 代替默认F0及升降调"), visible=False)
                    
                        file_index1e = gr.Textbox(
                    ...
                    with gr.Group():
                        with gr.Column(scale=2):
                            gr.Markdown(value='Comenzar la inferencia')

                            comenzar_inferencia_pro = gr.Button('Convertir', variant="primary")
                            gr.Markdown(value='Resultados')

                            area_estado = gr.Textbox(
                                value='No hay una inferencia en proceso.',
                                label='Estado de la inferencia:',
                                interactive=False,
                                show_label=True
                            )

                            with gr.Column():
                                sin_instrumental = gr.Audio(
                                    label='Resultado sin instrumental',
                                )

                                con_instrumental = gr.Audio(
                                    interactive=False,
                                    show_label=True,
                                    label='Resultado con instrumental',
                                    format=['wav', 'mp3']
                                )

                                

                                comenzar_inferencia_pro.click(
                                    fn=comenzar_inferencia,
                                    inputs=[
                                        spk_item1,
                                        fake_vision,
                                        vc_transform0,
                                    ...
                                    outputs=[area_estado, sin_instrumental, con_instrumental]
                                )       
                ...
                with gr.Row():
                    with gr.Column():
                        ...
                        enlace_drive = gr.Textbox(
                            ...
                            label='Enlace de drive del modelo:',
                            info='Enlace de drive del modelo que quieres descargar',
                            show_label=True,
                            visible=True,
                            interactive=True
                        )
                        ...
                        output_descarga = gr.Textbox(
                            value='',
                            label='Resultado de la descarga:',
                            interactive=False,
                            show_label=True                        
                        )

                        descargar_drive = gr.Button('Descargar', variant="primary")

                        descargar_drive.click(
                            fn=descargar_modelo_extraer,
                            inputs=[enlace_drive],
